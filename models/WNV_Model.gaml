...
global{
	...
	date starting_date <- date("2021-06-08-00-00-00");	
	//int charge_time <- 30;
	float alpha<-0.1;  
	 //mosquitoes

	int infectMos_time <- 2 ;
	//int MostiqueGlobal<-10 update:sum(Region collect each.Mostique);
...
species Region {
	...
	int nb_birds;
	int nb_birds_infected<-1.0 update:true;
	int nb_birds_sain;
...
output {	
	...
	//monitor "Number of Mosquitoes" value: MostiqueGlobal ; //nb_mosquitoes ;
	//monitor "Nombre of Infected Mosquitoes" value: MostiqueGlobalInfected ; //nb_mosquitoes ;
	//monitor "Nombre Total of Mosquitoes" value: sum(Region collect each.Mostique) color: #brown;
	monitor "Number Total f Infected Birds" value: birds count (each.is_infected) color: #red;
    ...
    display "Statistic par région" {   
			chart 'Number of Mosquitoes Nord EST' type: series {
				...
				data "Nombre Mos Manouba" value: r3.Mostique  color: #yellow marker_shape:  marker_circle;
				data "Nombre Mos Ben Arouss" value: r4.Mostique  color: #black marker_shape:  marker_circle;
				//data "Nombre of Infected Mos Ariana" value: r2.MostiqueI  color: #red marker_shape:  marker_circle;
